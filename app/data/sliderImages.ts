export const data = [
  {
    id: 1,
    src: 'https://images.pexels.com/photos/1639562/pexels-photo-1639562.jpeg?auto=compress&cs=tinysrgb&w=600',
    alt: 'Selective Focus Photo of Cheeseburger',
    headline: 'Delicious Cheeseburger',
    paragraph:
      'Enjoy our mouthwatering cheeseburger made with the finest ingredients.',
    cta: 'Order Now',
  },
  {
    id: 2,
    src: 'https://images.pexels.com/photos/4773769/pexels-photo-4773769.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
    alt: 'Person Holding a Pizza',
    headline: 'Authentic Italian Pizza',
    paragraph:
      'Indulge in the flavors of our authentic Italian pizza baked to perfection.',
    cta: 'Try Now',
  },
  {
    id: 3,
    src: 'https://images.pexels.com/photos/15832874/pexels-photo-15832874/free-photo-of-ready-to-eat-pasta-with-basil-and-cherry-tomatoes.jpeg?auto=compress&cs=tinysrgb&w=600',
    alt: 'Cooked Pasta on Brown Wooden Bowl',
    headline: 'Savory Pasta Delights',
    paragraph:
      'Treat yourself to our savory pasta dishes crafted with love and passion.',
    cta: 'Explore Menu',
  },
];
